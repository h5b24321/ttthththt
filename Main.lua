--[[
	...
	Modified: Dark Blue Theme + Loading Screen
	Original by Late
]]
...
--// YENİ KOYU MAVİ TEMA (Dark Blue Theme)
local Theme = { 
	--// Frames:
	Primary = Color3.fromRGB(18, 22, 35),       -- Arkaplan (Çok koyu mavi)
	Secondary = Color3.fromRGB(24, 28, 45),     -- Kenar çubukları ve başlıklar
	Component = Color3.fromRGB(32, 38, 60),     -- Butonlar ve bileşenler
	Interactables = Color3.fromRGB(45, 80, 160), -- Tıklanabilir/Aktif öğeler (Parlak Mavi)

	--// Text:
	Tab = Color3.fromRGB(160, 175, 200),        -- Tab yazıları (Grimsi mavi)
	Title = Color3.fromRGB(240, 245, 255),      -- Başlıklar (Beyazımsı mavi)
	Description = Color3.fromRGB(140, 155, 180),-- Açıklamalar

	--// Outlines:
	Shadow = Color3.fromRGB(5, 8, 15),          -- Gölgeler
	Outline = Color3.fromRGB(45, 55, 85),       -- Çerçeve çizgileri

	--// Image:
	Icon = Color3.fromRGB(200, 220, 255),       -- İkonlar
}
...
task.spawn(function()
	...
	LoadGui.Name = "LateLibLoading"
	LoadGui.IgnoreGuiInset = true
	LoadGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	
	if gethui then LoadGui.Parent = gethui() else LoadGui.Parent = game.CoreGui end
	...
	Logo.Image = "rbxassetid://7072729517" -- Shield icon generic
	Logo.ImageColor3 = Theme.Interactables
	...
	Spinner.BackgroundTransparency = 1.000
	...
	Stroke.Color = Theme.Interactables
	...
	-- Animation
	Services.Tween:Create(Logo, TweenInfo.new(0.5), {ImageTransparency = 0}):Play()
	Services.Tween:Create(Title, TweenInfo.new(0.5), {TextTransparency = 0}):Play()
	Services.Tween:Create(Stroke, TweenInfo.new(0.5), {Transparency = 0.5}):Play()
	...
	-- Fade Out
	Services.Tween:Create(MainFrame, TweenInfo.new(0.5), {BackgroundTransparency = 1}):Play()
	Services.Tween:Create(Logo, TweenInfo.new(0.5), {ImageTransparency = 1}):Play()
	Services.Tween:Create(Title, TweenInfo.new(0.5), {TextTransparency = 1}):Play()
	Services.Tween:Create(Stroke, TweenInfo.new(0.5), {Transparency = 1}):Play()
...
function Library:CreateWindow(Settings: { Title: string, Size: UDim2, Transparency: number, MinimizeKeybind: Enum.KeyCode?, Blurring: boolean, Theme: string })
	...
	function Options:AddToggle(Settings: { Title: string, Description: string, Default: boolean, Tab: Instance, Callback: any }) 
		...
		local Set = function(Value)
			if Value then
				Tween(Main,   .2, { BackgroundColor3 = Theme.Interactables }); -- Changed to Theme.Interactables for blue consistency
				Tween(Circle, .2, { BackgroundColor3 = Color3.fromRGB(255, 255, 255), Position = UDim2.new(1, -16, 0.5, 0) });
			else
				Tween(Main,   .2, { BackgroundColor3 = Theme.Component }); -- Changed to Theme.Component
				Tween(Circle, .2, { BackgroundColor3 = Theme.Tab, Position = UDim2.new(0, 3, 0.5, 0) });
			end
	...
	function Options:AddSlider(Settings: { Title: string, Description: string, MaxValue: number, AllowDecimals: boolean, DecimalAmount: number, Tab: Instance, Callback: any }) 
		...
		Fill.BackgroundColor3 = Theme.Interactables -- Slider Color
				...
				if Label:IsA("Frame") then
					...
					elseif Label.Parent:FindFirstChild("Value") then
						...
						if not Toggle.Value then
							Label.BackgroundColor3 = Theme.Component -- Fix for toggle background
							Label.Circle.BackgroundColor3 = Theme.Tab
						end
					else
